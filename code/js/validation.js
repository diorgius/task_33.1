...
async function validation(e) {
    if (e.target.id === 'email') {
        ...
        // проверка уникальности ввода имени группы
        data = {
            action: 'checkUserData',
        ...
        try {
            ...
            // console.log('Успех: ', result);
            if (result) {
                e.target.classList.add('wrong-data');
                document.querySelector('#btnaddgroup').setAttribute('disabled', '');
                // выводим сообщение о не соответствии введенных данных
                alertMessage(result);
            } else {
                e.target.classList.remove('wrong-data');
                document.querySelector('#btnaddgroup').removeAttribute('disabled');
            }
        } catch (error) {
            console.log('Ошибка: ', error);
        }
