...
async function validation(e) {
    ...
    if (e.target.id === 'email') {
        ...
        if (!emailregexp.test(email)) {
            inputemail.classList.add('wrong-pass')
            ...
            inputemail.classList.remove('wrong-pass')
            ...
            try {
                ...
                let response = await fetch('../app/core/CheckEmail.php', {
                    method: 'POST',
                    headers: {
        ...
        if (pass.length < 8 || pass.length > 20) {
            ...
            inputpassword.classList.add('wrong-pass')
            palert = document.createElement('p')
            palert.setAttribute('id', 'alert');
            buttonsend.setAttribute('disabled', '')
            divalert.appendChild(palert)
            ...
            inputpassword.classList.remove('wrong-pass')
            buttonsend.removeAttribute('disabled')
            if (document.querySelector('#alert')) {
                palert.textContent = ''
            }
        ...
        if (pass !== passagain) {
            ...
            inputpasswordagain.classList.add('wrong-pass')
            palert = document.createElement('p')
            palert.setAttribute('id', 'alert');
            buttonsend.setAttribute('disabled', '')
            divalert.appendChild(palert)
            ...
            inputpasswordagain.classList.remove('wrong-pass')
            buttonsend.removeAttribute('disabled')            
            if (document.querySelector('#alert')) {
                palert.textContent = ''
            }
