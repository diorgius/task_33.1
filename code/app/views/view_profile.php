        ...
        <div class="div-editprofile-form">
            <form class="form-editprofile" action="/profile/updateuser" method="post" enctype="multipart/form-data">
                <label for="email">Email:&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
                    <input class="input-editprofile input-short" type="email" id="email" name="email"
                        value="<?= $data['email'] ?>" readonly />&nbsp&nbsp
            ...
            <button class="btn-editprofile" type="submit"
                onclick="location.href='/profile/deleteuser/<?= $data['id'] ?>'">Удалить профиль</button>
        </div>
    </div>
<?php endif; ?>